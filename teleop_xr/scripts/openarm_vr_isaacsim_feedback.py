#!/usr/bin/env python3
"""
VR teleoperation for OpenArm bimanual robot in IsaacSim with feedback.
...
# IsaacLab USD kept as reference; it does not include the robot camera prims
# that we use for the Quest video panels.
...
def _drive_joint(stage: Usd.Stage, prim_path: str, angle_rad: float, is_finger: bool = False):
    """Set angular drive target (converts radians → degrees for USD DriveAPI)."""
        ...
        while app.is_running():
            ...
            if cmd:
                ...
                if cmd_count == 1 or cmd_count % 100 == 0:
                    ...
                    print(f"[Teleop] cmd #{cmd_count}: {sample}...")
            if active_cmd:
                for joint_name, angle_rad in active_cmd.items():
                    ...
                    if joint_name == "openarm_left_finger_joint1":
                        mirror_path = joint_map.get("openarm_left_finger_joint2")
                        if mirror_path:
                            _drive_joint(stage, mirror_path, angle_rad, is_finger=True)
                    elif joint_name == "openarm_right_finger_joint1":
                        mirror_path = joint_map.get("openarm_right_finger_joint2")
                        if mirror_path:
                            _drive_joint(stage, mirror_path, angle_rad, is_finger=True)
