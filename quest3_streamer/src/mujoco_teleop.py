#!/usr/bin/env python3
"""
Quest 3 → MuJoCo OpenArm Bimanual Teleoperation (local viewer, no browser)

Controls:
  GRIP (side squeeze)  — deadman switch, must hold to move arm (per-arm independent)
...
Run:
    ...
    python src/mujoco_teleop.py              # deadman ON (default, safer)
    python src/mujoco_teleop.py --no-deadman # arms always enabled

Joy message layout (from webxr_ros_bridge.py):
    axes[0] = trigger (index)
    axes[1] = squeeze / grip  ← deadman
"""
...
MODEL_PATH   = os.path.realpath(os.path.join(
    PROJECT_ROOT, "..", "openarm", "openarm", "simulation", "models", "scene_openarm.xml"))

# ── Tuning ────────────────────────────────────────────────────────────────────
SCALE          = 1.0    # hand-to-robot scale (1.0 = 1:1, metres)
SMOOTH         = 0.4    # IK target smoothing  (0 = raw, 0.9 = very smooth)
CALIB_FRAMES   = 60     # frames still to complete initial calibration
CALIB_STILL_M  = 0.015  # max jitter (m) that counts as "still"
DEADMAN_THRESH = 0.3    # squeeze axis threshold to enable arm (0–1)
IK_GAIN        = 0.5    # DLS step gain per iteration
IK_DAMP        = 0.005  # DLS damping
IK_ITERS       = 5      # IK iterations per viewer frame
MAX_STEP_M     = 0.05   # max IK target movement per frame (clamps spikes)
...
class ArmState:
    def __init__(self, name: str):
        ...
        self.anchor_vr     = None   # VR position when grip was last pressed
        self.anchor_robot  = None   # robot EE position at that moment
...
class TeleopNode(Node):
    def __init__(self, no_deadman: bool):
        super().__init__("quest_mujoco_teleop")
...
# ─────────────────────────────────────────────────────────────────────────────
def main():
    parser = argparse.ArgumentParser(description="Quest3 → MuJoCo bimanual teleop")
    parser.add_argument("--no-deadman", action="store_true",
                        help="Disable deadman switch — arms always follow")
    ...
    lf_qpos = model.jnt_qposadr[mujoco.mj_name2id(
        model, mujoco.mjtObj.mjOBJ_JOINT, "openarm_left_finger_joint1")]
    rf_qpos = model.jnt_qposadr[mujoco.mj_name2id(
        model, mujoco.mjtObj.mjOBJ_JOINT, "openarm_right_finger_joint1")]

    # FK at zero config → home positions
    mujoco.mj_forward(model, data)
    left_home  = get_body_pos(model, data, LEFT_TCP)
    right_home = get_body_pos(model, data, RIGHT_TCP)
    print(f"[FK] Left  home : {np.round(left_home,  3)}")
    print(f"[FK] Right home : {np.round(right_home, 3)}")
    ...
    if args.no_deadman:
        ...
        print("  Release GRIP → arm freezes in place (safe)")
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        ...
        while viewer.is_running():
            ...
            with node.lock:
                ...
                l_grip    = node.left.gripper
                r_grip    = node.right.gripper
            ...
            for arm, raw, enabled in [
                ...
                # Clamp per-frame step to prevent jumps on fast movement
                if arm.smooth_pos is not None:
                    step = desired - arm.smooth_pos
                    step_norm = np.linalg.norm(step)
                    if step_norm > MAX_STEP_M:
                        desired = arm.smooth_pos + step * (MAX_STEP_M / step_norm)
            ...
            # ── Gripper ───────────────────────────────────────────────────────
            data.qpos[lf_qpos] = float(np.clip(l_grip, GRIPPER_CLOSED, GRIPPER_OPEN))
            data.qpos[rf_qpos] = float(np.clip(r_grip, GRIPPER_CLOSED, GRIPPER_OPEN))
