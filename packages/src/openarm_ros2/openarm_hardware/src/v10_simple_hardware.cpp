  ...
  namespace {
  // Offset for physical RIGHT arm joint 3.
  // In bimanual launch, right arm is arm_prefix_ == "right_".
  // Local joint index 2 means joint3 because indexes are 0-based.
  // Mechanical zero measured from motor terminal: -1.127832 rad.
  constexpr size_t RIGHT_JOINT3_INDEX = 2;
  constexpr double RIGHT_JOINT3_OFFSET = -1.127832;

  // Convert raw motor position -> ROS/RViz position.
  double raw_to_ros_position(const std::string& arm_prefix, size_t joint_index,
                            double raw_position) {
    if (arm_prefix == "right_" && joint_index == RIGHT_JOINT3_INDEX) {
      return raw_position - RIGHT_JOINT3_OFFSET;
    }
    return raw_position;
  }

  // Convert ROS/RViz command -> raw motor command.
  double ros_to_raw_position(const std::string& arm_prefix, size_t joint_index,
                            double ros_position) {
    if (arm_prefix == "right_" && joint_index == RIGHT_JOINT3_INDEX) {
      return ros_position + RIGHT_JOINT3_OFFSET;
    }
    return ros_position;
  }
  }  // namespace
  ...
  hardware_interface::CallbackReturn OpenArm_v10HW::on_init(
    ...
    // Register ROS2 parameters so gains can be changed at runtime:
    //   ros2 param set /controller_manager left_kp1 95.0
    register_param_callback();
  ...
  for (size_t i = 0; i < ARM_DOF && i < arm_motors.size(); ++i) {
      const double raw_position = arm_motors[i].get_position();
      pos_states_[i] = raw_to_ros_position(arm_prefix_, i, raw_position);
  ...
  hardware_interface::return_type OpenArm_v10HW::write(
    ...
    for (size_t i = 0; i < ARM_DOF; ++i) {
    const double raw_position_command =
        ros_to_raw_position(arm_prefix_, i, pos_commands_[i]);

    arm_params.push_back(
        {kp_[i], kd_[i], raw_position_command, vel_commands_[i], tau_commands_[i]});
    }
  ...
  void OpenArm_v10HW::register_param_callback() {
    auto node = get_node();

    // Declare one parameter per joint gain, prefixed by arm_prefix_ so left/right
    // don't collide on the shared controller_manager node.
    // e.g. left_kp1, left_kd1, ..., right_kp7, right_kd7
    for (size_t i = 1; i <= ARM_DOF; ++i) {
      node->declare_parameter(arm_prefix_ + "kp" + std::to_string(i), kp_[i - 1]);
      node->declare_parameter(arm_prefix_ + "kd" + std::to_string(i), kd_[i - 1]);
    }
    node->declare_parameter(arm_prefix_ + "gripper_kp", gripper_kp_);
    node->declare_parameter(arm_prefix_ + "gripper_kd", gripper_kd_);

    RCLCPP_INFO(rclcpp::get_logger("OpenArm_v10HW"),
                "Gains registered as ROS2 params under prefix '%s'. "
                "Change at runtime with: ros2 param set /controller_manager %skp1 95.0",
                arm_prefix_.c_str(), arm_prefix_.c_str());
    ...
    param_callback_handle_ = node->add_on_set_parameters_callback(
      ...
      -> rcl_interfaces::msg::SetParametersResult {
        ...
        for (const auto & p : params) {
          ...
          const std::string kp_pfx = arm_prefix_ + "kp";
          const std::string kd_pfx = arm_prefix_ + "kd";
          if (name.rfind(kp_pfx, 0) == 0) {
            try {
              int idx = std::stoi(name.substr(kp_pfx.size())) - 1;
              if (idx >= 0 && idx < static_cast<int>(ARM_DOF)) {
                kp_[idx] = p.as_double();
                RCLCPP_INFO(rclcpp::get_logger("OpenArm_v10HW"),
                            "Updated %s = %.3f", name.c_str(), kp_[idx]);
              }
            } catch (...) {}
          } else if (name.rfind(kd_pfx, 0) == 0) {
            try {
              int idx = std::stoi(name.substr(kd_pfx.size())) - 1;
              if (idx >= 0 && idx < static_cast<int>(ARM_DOF)) {
                kd_[idx] = p.as_double();
                RCLCPP_INFO(rclcpp::get_logger("OpenArm_v10HW"),
                            "Updated %s = %.3f", name.c_str(), kd_[idx]);
              }
            } catch (...) {}
          } else if (name == arm_prefix_ + "gripper_kp") {
            gripper_kp_ = p.as_double();
            RCLCPP_INFO(rclcpp::get_logger("OpenArm_v10HW"),
                        "Updated gripper_kp = %.3f", gripper_kp_);
          } else if (name == arm_prefix_ + "gripper_kd") {
            gripper_kd_ = p.as_double();
            RCLCPP_INFO(rclcpp::get_logger("OpenArm_v10HW"),
                        "Updated gripper_kd = %.3f", gripper_kd_);
          }
        }
        return result;
      });
  ...
  void OpenArm_v10HW::return_to_zero() {
    ...
    // }
    for (size_t i = 0; i < ARM_DOF; ++i) {
    const double raw_zero_command = ros_to_raw_position(arm_prefix_, i, 0.0);
    arm_params.push_back({kp_[i], kd_[i], raw_zero_command, 0.0, 0.0});
    }
    openarm_->get_arm().mit_control_all(arm_params);
