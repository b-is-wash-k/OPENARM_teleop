...
def robot_nodes_spawner(
    ...
    # Jazzy ros2_control requires robot_description on the /robot_description
    # topic with TRANSIENT_LOCAL+RELIABLE QoS. robot_state_publisher may publish
    # it with incompatible QoS depending on the installed version, so we publish
    # it explicitly here with the correct QoS profile.
    desc_file = tempfile.NamedTemporaryFile(
        mode='w', suffix='.urdf', delete=False, prefix='openarm_rd_'
    )
    desc_file.write(robot_description)
    desc_file.close()
    ...
    rd_pub_code = (
        ...
        'rclpy.init(); '
        'n = Node("_robot_description_pub"); '
        'q = QoSProfile(depth=1, '
        '    durability=DurabilityPolicy.TRANSIENT_LOCAL, '
        '    reliability=ReliabilityPolicy.RELIABLE); '
        'p = n.create_publisher(String, "robot_description", q); '
        f'time.sleep(0.2); '
        f'p.publish(String(data=open("{desc_file.name}").read())); '
        'rclpy.spin(n)'
    )
    rd_publisher = ExecuteProcess(
        cmd=['python3', '-c', rd_pub_code],
        output='screen',
    )

    return [robot_state_pub_node, rd_publisher, control_node]
...
def generate_launch_description():
    ...
    delayed_arm_ctrl = TimerAction(
        period=6.0, actions=[controller_spawner_func])
    delayed_gripper = TimerAction(period=6.0, actions=[gripper_spawner])
