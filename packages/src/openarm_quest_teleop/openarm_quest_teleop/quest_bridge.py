#!/usr/bin/env python3
"""
Quest 3 → OpenArm Bimanual Teleoperation Bridge
...
class QuestBridge(Node):
    ...
    def process_quest_data(self, data: dict):
        ...
        # Debug: Log grip values to see if we're getting data
        left_grip = data.get("left", {}).get("grip", 0.0)
        right_grip = data.get("right", {}).get("grip", 0.0)
        self.get_logger().info(f"Received grip values - left: {left_grip:.2f}, right: {right_grip:.2f}, deadman_thresh: {self.deadman_thresh}")
