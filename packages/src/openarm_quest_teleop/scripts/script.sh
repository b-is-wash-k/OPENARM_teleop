...
#
#   cd ~/OPEN_ARM/packages/src/openarm_quest_teleop
...
#   3. Confirm the WebSocket field is ws://<same-computer-ip>:9090.
...
#   - The bridge is a plain ws:// WebSocket server. If a browser blocks ws://
#     from an HTTPS page, the ROS side can still be tested with ros2 topic echo,
#     but the web/bridge connection may need a future WSS update.
#
# Stop everything:
#
...
LOG_DIR="${LOG_DIR:-/tmp/openarm_quest_teleop_$(date +%Y%m%d_%H%M%S)}"
...
main() {
  ...
  start_process \
    ...
    ros2 run openarm_quest_teleop quest_bridge \
    --ros-args \
    -p ws_port:="${WS_PORT}"
