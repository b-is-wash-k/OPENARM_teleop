...
class QuestServoBoth(Node):
    ...
    def run(self):
        ...
        # Create ROS Rate for precise loop timing (same as demo_twist.cpp)
        rate = self.create_rate(100)  # 100Hz = 10ms period
    ...
    def publish_quest_twist_left(self):
        ...
        # 하드코딩 좌표 변환: Quest X,Y → Robot Y,X (x,y swap)
        current_pos = np.array([pos['y'], pos['x'], pos['z']])
        current_quat = np.array([rot['y'], rot['x'], rot['z'], rot['w']])
        ...
        # Calculate velocity (already in robot frame due to hardcoded swap)
